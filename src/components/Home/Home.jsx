    ...
    >
      <div className={`sliderWrapper flex w-[500vw]`} style={storeData}>
        {slideData.map((data, index)=>(
          ...
          <h1 className='sliderTitle absolute text-white top-[12%] sm:top-[30%] left-[7%] sm:left-[74%] lg:left-[76%] sm:w-[250px] sm:text-center sm:text-[26px] lg:text-[40px] leading-[1.2] font-bold uppercase'>{data.title} <br /> New <br /> Season</h1>
          <div className={`slidePrice absolute ${data.text} top-[40%] left-[8%] sm:left-[6%] lg:left-[10%] text-center text-[5vw] sm:text-[4vw] lg:text-[5vw]`}>
          <p>{data.price}</p>
          <div className={`priceProgressBar bg-gray-300 h-[2px] sm:h-[4px] mt-[-4px] sm:mt-[-14px]`}></div>
          </div>
          <button className="buyButton absolute text-white top-[70%] sm:top-[65%] right-[3%] sm:right-[5%] lg:right-[8%] text-center text-xs sm:text-[20px] lg:text-[28px] font-black border border-gray-500 cursor-pointer bg-black py-1 sm:py-4 px-1 sm:px-4 hover:text-black hover:bg-white rounded-lg">Buy Now!</button>
        </div>
        ))}
      </div>
    </div>
        ...
        {productData.map((data, index)=>(
          ...
          className="product w-full h-[240px] sm:h-[500px]">
            ...
            <div className="productDetails absolute top-[8%] sm:top-[2%] right-[2%] w-[40%] p-[20px] sm:p-[50px]">
              <h1 className=" productTitle text-[18px] sm:text-[60px] font-bold text-white sm:tracking-[-2px] leading-[1] capitalize">{data.title}</h1>
            ...
            <button className="productButton cursor-pointer bg-black text-white text-[12px] sm:text-[18px] rounded-lg py- sm:py-2 px-1 sm:px-4 border border-gray-300 absolute bottom-[22%] sm:bottom-[18%] right-[22%] sm:right-[30%] hover:text-black hover:bg-white hover:font-semibold" 
            // onClick={()=>{setBuyPanel({transform: 'scale(1)', transition: 'all ease 0.4s'})}}
            onClick={()=>{dispatch(fetchCartData(data.title))}}
            >Buy Now</button>
